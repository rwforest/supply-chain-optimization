...
Notebook `09_mps_cuopt_pipeline` goes beyond stress-testing the LP: it derives three classic operations-research problems from the MPS-like network and solves them — **FJSP** (flexible job-shop scheduling of back-end assembly/test) and **CVRPTW** (capacitated vehicle routing with time windows for distribution) on **NVIDIA cuOpt's GPU solvers**, plus **MEIO** (multi-echelon inventory optimization via the Guaranteed Service Model) on the existing Pyomo/HiGHS **CPU** stack. The problems are derived from the generated network (`scripts/mps_derivation.py`), not invented from scratch. A `scale` widget runs `09` on either the ~2,300-node complex MPS network or the ~76,000-node planet-scale one: FJSP machines and CVRPTW depots stay scale-invariant (keyed off MPS's real named back-end anchors, not the synthetic fan-out), and MEIO subsamples the network — always keeping tier-1, named anchors, and monopoly/oligopoly nodes — to stay CPU-tractable (the full 76k-node GSM MILP takes ~23 min in HiGHS; a ~2,500-node sample solves in seconds within ~0.6% of the exact objective). See the [MPS cuOpt pipeline guide](docs/mps-cuopt-pipeline.md).
